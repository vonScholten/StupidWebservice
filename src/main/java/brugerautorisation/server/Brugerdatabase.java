...
public class Brugerdatabase implements Serializable {
    ...
    public static void indlæsDeltagerlisteFraCampusnetHtml(String data, ArrayList<Bruger> brugere) {
        ...
        for (String tr : data.split("<tr")) {
            if (tr.contains("context_header")) {
                continue;
            }
            String td[] = tr.split("<td");
            if (td.length != 6) {
                continue; // Der er 6 kolonner i det, vi er interesserede i
            }
            ...
            if (b.email.contains("flhan@dtu.dk") || b.email.contains("phso@dtu.dk")) {
                continue; // drom adm personale
            }
            if (b.email.contains("feni@dtu.dk")) {
                continue; // drom adm personale
            }
            b.brugernavn = b.email.split("@")[0];
...
map={img=, name=Ahmad Mohammad Hassan Almajedi, email=s153317@student.dtu.dk s153317@student.dtu.dk, additional user-information=s153317, informationbox=id="participantinformation220040">, placeholder=, Brugernavn=s153317, Email=s153317@student.dtu.dk, Uddannelse=diploming. Softwaretek.}
            ...
            if (b.studeretning == null) {
                b.studeretning = "Underviser";
            } else if (b.studeretning.isEmpty()) {
                b.studeretning = "IT-Økonomi"; // Hvorfor ITØ'ernes er tom ved jeg ikke....
            } else {
                b.studeretning = b.studeretning.replace("diploming. ", "").replaceAll("[ \n]+", " ").trim();
            }
            b.adgangskode = "kode" + Integer.toString((int) (Math.random() * Integer.MAX_VALUE), Character.MAX_RADIX);
    ...
    public void gemTilFil(boolean tvingSkrivning) {
        if (!tvingSkrivning && filSidstGemt > System.currentTimeMillis() - 60000) {
            return; // Gem højst 1 gang per minut
        }		// Lav en sikkerhedskopi - i fald der skal rulles tilbage eller filen blir beskadiget
        try {
            if (!Files.exists(SIKKERHEDSKOPI)) {
                Files.createDirectories(SIKKERHEDSKOPI);
            }
    ...
    public Bruger ændrAdgangskode(String brugernavn, String glAdgangskode, String nyAdgangskode) {
        ...
        if (nyAdgangskode.isEmpty()) {
            throw new IllegalArgumentException("Ny adgangskode må ikke være tom");
        }
        if (nyAdgangskode.contains("\"") || nyAdgangskode.contains("'")) {
            throw new IllegalArgumentException("Ugyldige tegn i ny adgangskode");
        }
